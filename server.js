// server.js - Johnson Dictionary Feed Backend v5
...
const SERVER_VERSION = 'johnson-backend-v6-network-discovery';
...
async function fetchOneJohnsonEntry(browser, index = 0) {
  ...
  try {
    ...
    page.on('request', (req) => {
      ...
      // Block heavy resources. The image URL is still present in the DOM via img.src.
...
app.get('/debug/johnson', async (req, res) => {
  try {
    ...
    res.json({ ok: true, version: SERVER_VERSION, elapsedMs: Date.now() - started, entry });
  } catch (err) {
    await resetBrowser().catch(() => {});
    res.status(500).json({ ok: false, version: SERVER_VERSION, error: err.message || String(err) });
  }
});
...
app.get('/debug/network', async (req, res) => {
  ...
  try {
    ...
    page.on('response', async resp => {
      ...
      // Capture only small samples from likely data responses.
      if (['xhr', 'fetch', 'document', 'script'].includes(resourceType)) {
        try {
          const txt = await resp.text();
          item.sample = String(txt || '').slice(0, 800);
        } catch (_) {}
      }
      responses.push(item);
    });
    ...
    const controls = await page.evaluate(() => {
      ...
      return Array.from(document.querySelectorAll('button, input[type=button], input[type=submit], a, [role=button]'))
        .map((el, idx) => ({ idx, tag: el.tagName, text: textOf(el), href: el.href || '', id: el.id || '', cls: el.className || '' }))
        .filter(x => /random|word|search|permalink/i.test([x.text, x.href, x.id, x.cls].join(' ')))
        .slice(0, 40);
    });
    ...
    await page.evaluate(() => {
      const textOf = (el) => (el?.innerText || el?.textContent || el?.value || el?.getAttribute('aria-label') || '').trim();
      const candidates = Array.from(document.querySelectorAll('button, input[type=button], input[type=submit], a, [role=button]'));
      const randomControl = candidates.find(el => /random\s+word|random/i.test(textOf(el)));
      if (randomControl) randomControl.click();
    });
    ...
    const scripts = await page.evaluate(() => Array.from(document.scripts).map(s => s.src || '[inline]').filter(Boolean));

    const interestingRequests = requests.filter(r =>
      ['xhr', 'fetch', 'document', 'script'].includes(r.resourceType) ||
      /random|word|search|json|xml|php|api|ajax/i.test(r.url)
    );

    const interestingResponses = responses.filter(r =>
      ['xhr', 'fetch', 'document', 'script'].includes(r.resourceType) ||
      /random|word|search|json|xml|php|api|ajax/i.test(r.url + ' ' + (r.sample || ''))
    );
    ...
    return res.json({
      ...
      controls,
      scripts,
      requests: interestingRequests,
      responses: interestingResponses,
      bodySample
    });
  } catch (err) {
    return res.status(500).json({ ok: false, version: SERVER_VERSION, error: err.message || String(err) });
  } finally {
...
console.log(`[BOOT] Starting Johnson Dictionary Feed API ${SERVER_VERSION}... (node ${process.version})`);
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[BOOT] Listening on 0.0.0.0:${PORT}`);
  getBrowser().then(() => console.log('[BOOT] Browser warmed')).catch(err => console.error('[BOOT] Browser warm failed:', err.message || err));
