...
const VERSION = 'johnson-backend-v9-structured-content';
...
function extractStructuredEntry(displayHtml, meta) {
  ...
  const entryIndexBase = normalizeText(meta?.label || entryWord);
  const entryIndex = entryIndexBase.includes(meta.year)
    ? entryIndexBase
    : `${entryIndexBase}, ${meta.year}`;
  ...
  if (!definitions.length) {
    const full = normalizeText($('body').text() || $.root().text());
    const withoutHeader = full
      .replace(/^Johnson's Dictionary Online\s*/i, '')
      .replace(entryIndex, '')
      .replace(entryWord, '')
      .replace(grammar, '')
      .replace(derivation, '')
      .trim();
    if (withoutHeader) definitions.push({ definition: compactForCard(withoutHeader, 600), examples: [] });
  }
